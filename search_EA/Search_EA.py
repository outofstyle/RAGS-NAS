...
parser.add_argument("--name", type=str, default="Tabular_Search_S_CMA_ES")
...
parser.add_argument('--save_dir',
                        ...
                        default='/output/nasbench_101',
                        type=str,
                        help='Path to save output')
parser.add_argument('--save_file_name',
                        default='result_CMA_nas101.log',  # 93.79006227
                        type=str,
                        help='save file name')
...
runfolder = f"NAS_Search_CMA_ES_{args.dataset}/{args.image_data}/{runfolder}_{args.name}_{args.dataset}_{args.seed}"
...
def train(
    ...
    for epoch in range(args.epochs):
        ...
        # mean.data = mean.data - args.lr * mean.grad  # vanilla gradient descent update
    ...
    with tqdm.tqdm(total=search_data, desc="Instances", unit="") as pbar:
        while True:
            ...
            if instances >= instances_total:
                ...
                ws_mean, ws_sigma, ws_cov = get_warm_start_mgd(source_solutions=all_solutions, gamma=0.2, alpha=0.2) ##0.1,0.1for cifar100
