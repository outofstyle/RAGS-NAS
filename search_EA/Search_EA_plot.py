...
from plot_tsne import visualize2D
from plot_pca import pca_vis
##############################################################################
#
#                              Arguments
#
...
parser.add_argument('--save_file_name',
                        default='result_CMA_plot.log',  # 93.79006227
                        type=str,
                        help='save file name')
    ...
    with torch.no_grad():
        ...
        while j < num:
            ...
            if v > 250000:
                ...
                ws_mean, ws_sigma, ws_cov = get_warm_start_mgd(source_solutions=solution, gamma=0.2, alpha=0.2)
                temp_optimizer = CMA(population_size=num, mean=ws_mean, bounds=bound, sigma=ws_sigma, cov=ws_cov)
                pop = []
                for k in range(num):
                ...
                noise = (-6) * torch.rand(num, latent_dim) + 3
...
##############################################################################
def train_ranker(ranker, optimizer, scheduler, batch_n, conditional_train_data, normal_cell):

    if normal_cell:
        encodings = torch.stack([arch.y_normal.cpu().detach() for arch in conditional_train_data])
        ...
        encodings = torch.stack([arch.y_reduce.cpu().detach() for arch in conditional_train_data])
...
def train(
    ...
    if normal_cell:
        ...
        encodings = real.y_normal.reshape(b_size,-1).cpu()
        ...
        encodings = real.y_reduce.reshape(b_size,-1).cpu()
...
def eval_ranker(ranker, test_data, best, normal_cell):
    ...
    if normal_cell:    
        encodings = torch.stack([arch.y_normal.cpu().detach() for arch in test_data])
    else:    
        encodings = torch.stack([arch.y_reduce.cpu().detach() for arch in test_data])
    ...
    for i, arch in enumerate(test_data):
        arch.acc = torch.FloatTensor([acq_candidates[i]])
...
def training_loop(train_data, ranker):
    ...
    tmp_visited = {}
    init_reduce_cell,_ = sample_one(G, measurements, args.device, args.dataset, num=1, normal=False)
    sampled_data = sample_data(G, init_reduce_cell, tmp_visited, measurements, args.device, args.dataset, 128, normal=False)
    ...
    sampled_X, sampled_y = [], []
    tmp_visited = {}
    while len(sampled_X) < 40000:
        sampled_reduce_cell,_ = sample_one(G, measurements, args.device, args.dataset, num=1, normal=False)
        sampled_data = sample_data(G, sampled_reduce_cell, tmp_visited, measurements, args.device, args.dataset, 128, normal=False)
        for i in range(len(sampled_data)):
            try:
                arch = sampled_data[i]
                arch = Dataset.get_info_generated_graph(arch, args.image_data)
                arch.to('cpu')
                sampled_X.append(arch.z)
                sampled_y.append(arch.acc)
                h = str(arch.y_normal.detach().tolist()+arch.y_reduce.detach().tolist())
                tmp_visited[h] = 1
                if len(sampled_X) >= 40000:
                    break
            except:
                continue
            
    #visualize2D(sampled_X, sampled_y, 0)
    pca_vis(sampled_X, sampled_y, 0)
    ...
    with tqdm.tqdm(total=search_data//2, desc="Instances", unit="") as pbar:
        while True:
            ...
            ranker = train_ranker(ranker, ranker_optimizer, ranker_scheduler, batch_n, conditional_train_data, normal_cell=True)
            #######################################
            ...
            for batch, w in weighted_dataloader:
                ...
                instances += b_size
            if len(conditional_train_data) == 208 :
                sampled_X, sampled_y = [], []
                tmp_visited = {}
                while len(sampled_X) < 40000:
                    sampled_reduce_cell,_ = sample_one(G, measurements, args.device, args.dataset, num=1, normal=False)
                    ...
                    for i in range(len(sampled_data)):
                        try:
                            ...
                            continue
                #visualize2D(sampled_X, sampled_y, len(conditional_train_data))
                pca_vis(sampled_X, sampled_y, len(conditional_train_data))
            if instances >= instances_total:
                ...
                ws_mean, ws_sigma, ws_cov = get_warm_start_mgd(source_solutions=source_solutions, gamma=0.3, alpha=0.3)
    ...
    with tqdm.tqdm(total=search_data, desc="Instances", unit="") as pbar:
        while True:
            ...
            ranker = train_ranker(ranker, ranker_optimizer, ranker_scheduler, batch_n, conditional_train_data, normal_cell=False)
            #######################################
            upd = len(conditional_train_data)-pbar.n
            if upd > 0:
                pbar.update(upd)
            for batch, w in weighted_dataloader:
                ...
                instances += b_size
            if len(conditional_train_data) == 400 or len(conditional_train_data) == 608:
                sampled_X, sampled_y = [], []
                tmp_visited = {}
                while len(sampled_X) < 40000:
                    #sampled_reduce_cell,_ = sample_one(G, measurements, args.device, args.dataset, num=1, normal=False)
                    sampled_data = sample_data(G, [normal_cell], tmp_visited, measurements, args.device, args.dataset, 128, normal=True)
                    for i in range(len(sampled_data)):
                        try:
                            arch = sampled_data[i]
                            arch = Dataset.get_info_generated_graph(arch, args.image_data)
...
if __name__ == "__main__":
    ...
    for i in range(args.trials):
        ...
        ranker = Listwise_Ranker(input_dim=176, hidden_dim=512, hidden_layer=2)
        ##############################################################################
        #
        #                              Dataset
        #
