...
from datasets.nasbench.lib import base_ops
from datasets.nasbench.lib import training_time
import numpy as np
...
def build_model_fn(spec, config, num_train_images):
  ...
  def model_fn(features, labels, mode, params):
    ...
    if mode == tf.estimator.ModeKeys.PREDICT:
      ...
      return tf.compat.v1.contrib.tpu.TPUEstimatorSpec(mode=mode, predictions=predictions)
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
      ...
      optimizer = tf.train.compat.v1.RMSPropOptimizer(
